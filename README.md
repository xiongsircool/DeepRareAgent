...
**核心理念 (Core Philosophy)**：单一智能体系统易受模型幻觉与固有偏见影响，本系统引入**多团队集成 (Multi-Team Ensemble)** 机制，通过独立视角的交叉验证来降低诊断风险。鉴于通用 LLM 缺乏罕见病领域的特异性数据，系统集成了**领域专用工具链 (Domain-Specific Tools)**（如 HPO 本体映射、PubMed 实时检索）以突破预训练知识边界。此外，为应对复杂病例的信息过载与长上下文限制，系统模拟现实医疗团队中**"主治-专员" (Leader-Member)** 的分层协作模式，并结合**长效记忆 (Long-Term Memory)** 与**关键证据提取**技术，从而保障诊断推理的深度与逻辑连贯性。
